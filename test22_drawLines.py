...
robots = [{'x': 5, 'y': 5, 'detectado': False, 'angulo': 2 * np.pi *
           random.random(), 'distancia': 0, 'regresando': False, 'trayectoria': [(5, 5)]} for _ in range(num_robots)]
...
def mover_hacia_fuego(robot):
    ...
    robot['trayectoria'].append((robot['x'], robot['y']))
...
def actualizar(frame):
    ...
    for i, robot in enumerate(robots):
        ...
        # Dibujar la trayectoria de cada robot
        for i in range(len(robot['trayectoria']) - 1):
            ax.plot([robot['trayectoria'][i][0], robot['trayectoria'][i + 1][0]],
                    [robot['trayectoria'][i][1], robot['trayectoria'][i + 1][1]], 'k--')
